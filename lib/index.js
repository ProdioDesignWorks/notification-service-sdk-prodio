...
var getEvent = function getEvent(BASE_URL, callback) {
  var url = BASE_URL + '/events/event';
  axios.get(url).then(function (response) {
    return callback(response);
  }).catch(function (error) {
...
var getMessages = function getMessages(BASE_URL, callback) {
  var url = BASE_URL + '/message/message';
  axios.get(url).then(function (response) {
    return callback(response);
  }).catch(function (error) {
...
var sendMail = function sendMail(payload, BASE_URL, callback) {
  if (!isJson(payload)) {
    return callback(new HttpErrors.BadRequest('Payload must be a JSON object.', { expose: false }));
  } else {
    payload = payload.meta;
    if (isNull(payload.eventName)) {
      ...
      if (!isNull(payload.props)) {
        _payloadProps = payload.props;
      }
      var url = BASE_URL + '/notification-subscribers/email?eventName=' + payload.eventName + '&subscriberId=' + payload.subscriberId;
      axios.post(url, _payloadProps).then(function (response) {
        return callback(response);
      }).catch(function (error) {
        var json = CircularJSON.stringify(error);
        return callback(json);
...
var sendTransactionalMail = function sendTransactionalMail(payload, BASE_URL, callback) {
  if (!isJson(payload)) {
    ...
    if (isNull(payload.eventName)) {
      ...
      var _payloadProps2 = {};
      if (!isNull(payload.props)) {
        _payloadProps2 = payload.props;
      }
      var url = BASE_URL + '/notification-subscribers/transEmail?eventName=' + payload.eventName + '&subscriberId=' + payload.subscriberId;
      axios.post(url, _payloadProps2).then(function (response) {
        return callback(response);
      }).catch(function (error) {
        var json = CircularJSON.stringify(error);
        return callback(json);
...
var sendWebPushNotifications = function sendWebPushNotifications(payload, BASE_URL, callback) {
  if (!isJson(payload)) {
    ...
    if (isNull(payload.eventName)) {
      ...
      var _payloadProps4 = {};
      if (!isNull(payload.props)) {
        _payloadProps4.props = payload.props;
      }
      if (!isNull(payload.data)) {
        _payloadProps4.notificationData = payload.data;
      }
      var url = BASE_URL + '/notification-subscribers/webNotification?eventName=' + payload.eventName + '&subscriberId=' + payload.subscriberId;
      axios.post(url, _payloadProps4).then(function (response) {
        return callback(response);
      }).catch(function (error) {
        var json = CircularJSON.stringify(error);
        return callback(json);
